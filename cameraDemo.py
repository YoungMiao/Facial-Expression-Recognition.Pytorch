...
while True:
    ...
    bounding_boxes, landmarks = detect_faces(PILimage)

    #print(bounding_boxes[:])
    for bbox in bounding_boxes:
        det = np.squeeze(bbox[:4])
        
        # face rectangle
        det[0] = np.maximum(det[0] - margin, 0)
        det[1] = np.maximum(det[1] - margin, 0)
        det[2] = np.minimum(det[2] + margin, frame.shape[1])
        det[3] = np.minimum(det[3] + margin, frame.shape[0])
        ...
        cv2.putText(frame, 'emotion : %s' % (class_names[int(predicted.cpu().numpy())]), (bb[0] - 10, bb[1] - 10),
